...
function qs_quote_builder_shortcode() {
	...
	<h3>Specifications</h3><p class="qs-form-group"><label class="qs-label" for="pricing_type">Pricing mode</label><select class="qs-input" id="pricing_type" name="pricing_type"><option value="trade" <?php selected( $quote_id ? get_post_meta( $quote_id, '_pricing_type', true ) : 'trade', 'trade' ); ?>>Trade pricing</option><option value="retail" <?php selected( get_post_meta( $quote_id, '_pricing_type', true ), 'retail' ); ?>>Retail pricing (trade + 22.22%)</option></select></p><?php qs_builder_input( 'door_profile', 'Profile', $meta['door_profile'] ); qs_builder_input( 'timber', 'Timber', $meta['timber'] ); qs_builder_input( 'finish', 'Finish', $meta['finish'] ); qs_builder_input( 'handle_profile', 'Door / drawer handle profile', $meta['handle_profile'] ); ?>
	...
	<aside class="qs-summary"><h3>Quote summary</h3><p>Doors: <?php echo esc_html( qs_quote_component_count( $quote_id, 'doors_drawers', 'Door' ) ); ?></p><p>Drawers: <?php echo esc_html( qs_quote_component_count( $quote_id, 'doors_drawers', 'Drawer' ) ); ?></p><p>Panels: <?php echo esc_html( qs_quote_component_count( $quote_id, 'end_panels' ) ); ?></p><p>Fillers: <?php echo esc_html( qs_quote_component_count( $quote_id, 'fillers' ) ); ?></p><p>Kickboards: <?php echo esc_html( qs_quote_component_count( $quote_id, 'kickboards' ) ); ?></p><p><strong>Calculated total: $<?php echo esc_html( number_format( $quote_id ? qs_calculate_total( $quote_id ) : 0, 2 ) ); ?> AUD</strong></p><p><small>Prices are set by the office and update when you save this quote.</small></p></aside></form></div>
