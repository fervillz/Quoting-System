		...
		function renderSubtotalValue(value){
			...
			fetch(ajaxUrl,{method:'POST',body:data,credentials:'same-origin',signal:calculateRequest.signal})
				...
				.then(result=>{
					...
					if(subtotal)animateSubtotalTo(Number(String(result.data.formatted_subtotal).replace(/,/g,'')));
...
+Number(value||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})+' AUD';
		}
		function animateSubtotalTo(target){
			if(!subtotal)return;
