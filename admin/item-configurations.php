...
function setAdminFieldDisabled(container,disabled){if(!container)return;container.querySelectorAll('input,select,textarea').forEach(function(input){input.disabled=disabled;});}
function syncDrawer(row){var type=row.querySelector('[data-qs-item-type]'),bank=type&&type.value==='Drawer Bank',height=row.querySelector('.qs-admin-standard-height');if(height){height.hidden=bank;setAdminFieldDisabled(height,bank);}row.querySelectorAll('.qs-admin-drawer-bank-only').forEach(function(el){el.hidden=!bank;setAdminFieldDisabled(el,!bank);});if(!bank)return;var count=String((row.querySelector('[data-qs-drawer-count]')||{}).value||'3');row.querySelectorAll('.qs-drawer-height').forEach(function(label){var active=label.dataset.drawerCounts.split(' ').includes(count);label.hidden=!active;setAdminFieldDisabled(label,!active);if(!active){var input=label.querySelector('input');if(input)input.value='';}});}
